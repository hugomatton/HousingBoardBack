...
export const createHousing = async (req, res) => {
    const { housing_address,
            ...
            owner_id,
            picture_id } = req.body
    const sql = `INSERT INTO housing (housing_address, bedrooms_nb, bathrooms_nb, area, monthly_rent, lease_duration, furnished, type_name, owner_id, picture_id) 
                 VALUES (:housing_address, :bedrooms_nb, :bathrooms_nb, :area, :monthly_rent, :lease_duration, :furnished, :type_name, :owner_id, :picture_id)`;
    const bindParams = {housing_address, bedrooms_nb, bathrooms_nb, area, monthly_rent, lease_duration, furnished, type_name, owner_id, picture_id}
    try {
        const connection = await oracledb.getConnection(dbconfig);
        const result = await connection.execute(sql, bindParams);
